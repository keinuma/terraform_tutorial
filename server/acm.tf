...
resource "aws_acm_certificate_validation" "api_acm_certificate_validation" {
  ...
  validation_record_fqdns = [for record in aws_route53_record.api_cert_verification_r53_record : record.fqdn]
}

resource "aws_route53_record" "api_cert_verification_r53_record" {
  for_each = {
    for dvo in aws_acm_certificate.api_acm_certifacate.domain_validation_options : dvo.domain_name => {
